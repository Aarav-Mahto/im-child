...
const Header: React.FC = () => {
  ...
  const headerClasses = [
    ...
    isScrolled
      ? 'bg-black/20 backdrop-blur-xl text-neutral-900 shadow-lg py-2'
      : 'bg-black/0 text-neutral-900 backdrop-blur-xl border-b border-neutral-200 py-3'
  ].join(' ')
    ...
    <header className={headerClasses}>
      <Container>
        <div className="flex items-center justify-between">
          ...
          {/* Desktop Navigation */}
          <nav className="hidden lg:flex items-center md:gap-4 lg:gap-4 xl:gap-6 2xl:gap-8">
            <button
          ...
          >
            <div className="relative w-6 h-6">
              <span className={`absolute block h-0.5 w-full bg-neutral-900 rounded-sm opacity-100 left-0 transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'top-3 rotate-45' : 'top-0'}`}></span>
              <span className={`absolute block h-0.5 w-full bg-neutral-900 rounded-sm opacity-100 left-0 transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'opacity-0 -left-12' : 'top-2'}`}></span>
              <span className={`absolute block h-0.5 w-full bg-neutral-900 rounded-sm opacity-100 left-0 transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'top-3 -rotate-45' : 'top-4'}`}></span>
            </div>
          </button>
        </div>
        ...
        {/* Mobile Navigation */}
        <nav
          className={`block absolute top-full left-0 right-0 bg-white/95 text-neutral-900 backdrop-blur-xl border-b border-neutral-300 shadow-2xl transition-all duration-300 ease-out-cubic lg:hidden ${isMobileMenuOpen
